...
# 純粋ユーティリティ (_normalize, _clean_amount, _extract_core_name,
# _format_wareki, _parse_kouki, _serial_to_datetime ...) と
# Cell アクセスラッパー (_cell_str, _cell_raw, _cell_value_strip ...) は
# `extractor_utils.py` に分離。
#
# 依頼書スキャン群 (_detect_vendor_base_cols, _scan_keyword_rows,
# _find_sub_keyword_row, _extract_kingaku_direct と KINGAKU_KEYWORD_VARIANTS /
# _KINGAKU_LABELS) は `irai_scan_utils.py` に分離。
#
# 日付・工期パーサーは `extractor_utils.py` (_format_wareki / _parse_kouki /
# _parse_contract_date 等) を参照。
#
# いずれも re-export 経由で利用する。


# 業者×シートのマッチング (build_sheet_assignment, _match_score,
# _sheet_contains_vendor, _find_related_sheets, _extract_from_first_joken) と
# シート走査範囲定数 (_SHEET_SCAN_MAX_ROW / _SHEET_SCAN_MAX_COL) は
# `sheet_assignment_utils.py` に分離。
#
# 契約条件書テキストデータ抽出 (extract_joken_text_data) と
# VML チェックボックス解析 (_extract_checkboxes_from_vml 等) は
# `vml_utils.py` に分離。
#
# いずれも re-export 経由で利用する。
