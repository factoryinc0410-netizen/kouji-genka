...
### G-5. prod-app の `.env` `PORT` と systemd unit 引数の不整合（技術負債・小）

**発見経緯**:
Phase G-2 の本番監視構築時（2026-05-06、コミット `a0ffb61`）に判明。`prod-app/.env` には `PORT=8080` と記載されているが、`factory-prod.service` の `ExecStart` が `--port 8000` を明示的に渡すため実稼働は `:8000`。`.env` の `PORT` 値は事実上無視されている。

**影響**:
- 設定ファイルを読んだ運用者が「prod は `:8080` で動いている」と誤認するリスク
- `.env` の `PORT` を変更してもサービス再起動後にポートは変わらない（変更が無効な錯覚を与える）
- 現状は実害なし — 監視 cron も backup も systemd の実態（`:8000`）に整合済み

**着手内容**（どちらか選択）:
- **(A) systemd unit を `.env` 主導に統一**: `factory-prod.service` の `ExecStart` から `--port 8000` を削除し、`web_app/core/config.py` 経由で `PORT` を読ませる。`.env` を真の値とする。
- **(B) `.env` を実態に合わせる**: `prod-app/.env` の `PORT=8080` を `PORT=8000` に修正。systemd unit の引数を真の値とする方針。

**リスク**:
- (A): 中。systemd unit 編集 → サービス再起動が必要。再起動中に瞬断（数秒〜十数秒）。`.env` 読込ロジックが期待通り動くか事前検証必要。
- (B): 低。`.env` の値書き換えのみ（実稼働ポートは変わらない）。ただし「設定ファイル＝実態」の原則から見ると姑息な対処になる。

**推奨**: **(B)** をまず実施（即時・低リスク）→ 余裕があれば後日 (A) に移行して `.env` 主導へ。

**規模**: 小（(B) は 5 分、(A) は 30 分）。
